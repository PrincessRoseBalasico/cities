...
function averagePopulation(data) {
  // Extract the population values from the data array
const populations = _.map(cities, "population");

  // Use reduce to calculate the sum of populations
  const totalPopulation = _.reduce(populations, (sum, population) => sum + population, 0);
...
function biggestAndSmallest(data) {
  // Use max and min to find the cities with the highest and lowest populations
  const biggest = _.max(cities, city => city.population);
  const smallest = _.min(cities, city => city.population);
